...
import HtmlCode, { highlightHtml } from './HtmlCode'
...
export default function StepGuide() {
  ...
  return <section id="step-by-step" className="react-learning-section">
    ...
    <div className="react-step-nav" aria-label="เลือกขั้นตอน">{steps.map((item, i) => <button key={item.step} className={i === index ? 'active' : ''} aria-current={i === index ? 'step' : undefined} onClick={() => setIndex(i)}><span>{i < index ? <CheckCircle2 size={18} /> : item.step}</span><HtmlCode>{item.targetTag}</HtmlCode></button>)}</div>
    <div className="react-code-grid">
      <div className="react-panel"><span id="step-current-number">ขั้นตอนที่ {step.step} / {steps.length}</span><h3 id="step-current-title">{step.title}</h3><span className="react-badge">{step.badge}</span><p id="step-current-explanation">{step.explanation}</p><aside className="react-notice">{step.focusPoint}</aside>
        ...
        <pre className="react-code" id="step-code-content"><code>{step.code.split('\n').map((line, i) => <span className={step.highlightLines.includes(i + 1) ? 'highlight-line' : ''} key={i}><small>{i + 1}</small>{highlightHtml(line, `step-${index}-${i}`)}{'\n'}</span>)}</code></pre>
